...
const ExpiredUsers = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_BASE_URL}/api/user/expired-users`, {
      ...
      Swal.fire("Error", "Failed to fetch expired users.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="card space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-gray-400">Expired users</p>
          <h2 className="text-lg font-semibold dark:text-white">Members with expired plans</h2>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="rounded-xl bg-amber-100 px-4 py-2 text-sm font-semibold text-amber-700 dark:bg-amber-900/40 dark:text-amber-100">
            Total: {count}
          </div>
        </div>
      </div>
              ...
              {filtered.length > 0 ? (
                (() => {
                  ...
                  <tr key={user._id} className="table-row">
                    ...
                    <td className="table-cell">
                      <span className="rounded-full bg-gray-200 px-2 py-1 text-xs font-semibold text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                        Expired
                      </span>
                    </td>
                    <td className="table-cell">
