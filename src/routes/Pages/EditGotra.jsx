...
const EditGotra = () => {
  ...
  const [gotraData, setGotraData] = useState({ name: "", subCaste: "" });
  ...
  useEffect(() => {
    const fetchGotraAndSubCastes = async () => {
      ...
      setGotraData({
        name: gotra.name || "",
        subCaste: gotra.subCaste?._id || "",  // ✅ fixed here
      });
      ...
      <div className="card-body">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 flex justify-end">
            <button type="button" onClick={() => navigate("/gotra")} className="mr-4 py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
              Cancel
            </button>
